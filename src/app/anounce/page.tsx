...
const AnouncePage: React.FC = () => {
  ...
  const fetchAnnouncementsMaterial = async (material: string) => {
    if (material == "all"){fetchAnnouncements()}
    
    
    if (material != "all"){
    try {
      const response = await API.get(`/anounce/listbyresiduename/?skip=1&take=5&name=${material}`); // Rota da sua API
            console.log(response)
  ...
  return (
    <>
      ...
      <div className="py-2 text-2xl font-semibold flex">
        ...
        <div className="top-0">
        <h2  className="text-2xl text px-9 font-semibold text-white">Anounce</h2>
        ...
        </div>
        {
          data.filter((data: any) => {
              ...
              if (data.title.toLowerCase().includes(search.toLowerCase())) {
                ...
                key={item.id}
>                  <ProfileLink image={item.profile.image_url} username={item.profile.username}/>
