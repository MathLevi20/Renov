...
import ResidueCard from "@/components/sendPage/SendPageCard";
...
const Received: React.FC = () => {
    ...
    try {
      const response = await API.get('/proposal/myproposals/', {
        headers: {
  ...
  return (
    <>
      ...
      <Container maxW="container.xl" py={8} px={10}>
        <h2 className=" text text-slate-100  text-2xl text px-5 font-semibold">Propostas Enviadas</h2>
        ...
        {data
          ...
          <ResidueCard
            key={data.id}
              ...
              created_at ={format(new Date(data.created_at), 'dd/MM/yyyy HH:mm:ss')}
              id={data.id}
